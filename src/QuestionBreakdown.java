...
public class QuestionBreakdown
{
    ...
    public static void main(String[] args)
    {
        ...
        /*
        Map.Entry<ArrayList<String>, ArrayList<String>> origResults = fetchEcosiaTitlesAndSnippets
                (searchWords, 10);
        ArrayList<String> origTitles = origResults.getKey();
        ArrayList<String> origSnippets = origResults.getValue();

        ArrayList<String[]> alteredSearches = alternativeSearches(searchWords, 5);

        ArrayList<String> synTitles = new ArrayList<>();
        ArrayList<String> synSnippets = new ArrayList<>();

        for (String[] altSearch : alteredSearches)
        {
            Map.Entry<ArrayList<String>, ArrayList<String>> results = fetchEcosiaTitlesAndSnippets
                    (altSearch, 3);
            ArrayList<String> titles = results.getKey();
            ArrayList<String> snippets = results.getValue();
            synTitles.addAll(titles);
            synSnippets.addAll(snippets);
        }

        System.out.println(origTitles);
        System.out.println(origSnippets);

        System.out.println();

        System.out.println(synTitles);
        System.out.println(synSnippets);
        */
